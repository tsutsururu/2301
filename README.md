...
# ☆ 284E Count Simple Paths  diff 1043  計算量推定正しくできるか
...
頂点が探索される回数は 10回に抑えられるとは限らない。ある頂点へ同じ頂点から入ってきたとしてもそれらが同じパスとは限らない。そのため、パスの上限で抑える必要があった。また一つのパスが決定する(終点から到達帰る)までに終点の入次数回の計算量が生じるので、全体の計算量は O( 10* min(K,10^6) ) となる。 したがって、次数に制限がないと パスの上限で抑えていても計算回数はこれを上回ってしまう可能性がある。
